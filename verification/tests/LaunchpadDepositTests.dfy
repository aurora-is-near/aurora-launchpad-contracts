module LaunchpadDepositTests {
  ...
  function DepositFixedPriceWithoutRefundTest(): bool {
    ...
    var (lp2, amount2, weight2, refund2) := lp1.DepositSpec(alice, 100000, IntentAccount(alice), NOW);
    ...
    var (lp3, amount3, weight3, refund3) := lp2.DepositSpec(alice, 100000, IntentAccount(alice), NOW);
    assert amount3 == 100000;
    assert weight3 == 100000;
    assert refund3 == 0;
    assert lp3.totalDeposited == 200000;
    assert lp3.totalSoldTokens == 200000;
    assert lp3.investments[IntentAccount(alice)] == InvestmentAmount(200000, 200000, 0);
  ...
  function DepositFixedPriceWithRefundTest(): bool {
    ...
    var alice := "alice.testnet";
    var (lp2, amount2, weight2, refund2) := lp1.DepositSpec(alice, cfg.totalSaleAmount + 10000, IntentAccount(alice), NOW);
    ...
    assert lp2.investments[IntentAccount(alice)] == InvestmentAmount(cfg.totalSaleAmount, cfg.totalSaleAmount, 0);
  ...
  function DepositFixedPrice_1_2_WithRefundTest(): bool {
    ...
    assert lp2.totalSoldTokens == cfg.totalSaleAmount;
    assert lp2.investments[IntentAccount(alice)] == InvestmentAmount(cfg.totalSaleAmount / 2, cfg.totalSaleAmount, 0);
  ...
  function DepositMultipleAccountsFixedPrice_2_1_WithRefundTest(): bool {
    ...
    assert lp2.investments[IntentAccount(alice)] == InvestmentAmount(cfg.totalSaleAmount  + 10000, assets, 0);

    var amount_2 := 10000;
    var assets2 := CalculateAssetsSpec(cfg.CalculateWeightedAmountSpec(amount_2, NOW), 2, 1);
    var (lp3, amount3, weight3, refund3) := lp2.DepositSpec(alice, amount_2, IntentAccount(alice), NOW);
    ...
    assert lp3.investments[IntentAccount(alice)] == InvestmentAmount(cfg.totalSaleAmount  + 20000, assets + assets2, 0);

    var bob := "bob.testnet";
    var amount_3 := 190000;
    var assets3 := CalculateAssetsSpec(cfg.CalculateWeightedAmountSpec(180000, NOW), 2, 1);
    var (lp4, amount4, weight4, refund4) := lp3.DepositSpec(bob, amount_3, IntentAccount(bob), NOW);
    ...
    assert lp4.investments[IntentAccount(alice)] == InvestmentAmount(cfg.totalSaleAmount  + 20000, assets + assets2, 0);
    assert lp4.investments[IntentAccount(bob)] == InvestmentAmount(180000, assets3, 0);
  ...
  function DepositFixedPriceWithDiscountWithRefundTest(): bool {
    ...
    assert lp2.investments[IntentAccount(alice)] == InvestmentAmount(amount, 120000, 0);
  ...
  function DepositPriceDiscoveryWithDiscountTest(): bool {
    ...
    var timeInDiscount2Ended := NOW + 2000;
    ...
    var (lp1, amountToSale, weight1, refund1) := lp.DepositSpec(cfg.saleTokenAccountId, cfg.totalSaleAmount, IntentAccount(cfg.saleTokenAccountId), 100);
    ...
    // Applying discount1
    assert discount1.IsActive(timeInDiscount1);
    var alice := "alice.testnet";
    ...
    var (lp2, amount2, weight2, refund2) := lp1.DepositSpec(alice, amount, alice_intent_acc, timeInDiscount1);
    assert amount2 == amount;
    assert weight2 == expectedWeight1;
    assert refund2 == 0;
    assert lp2.totalDeposited == amount;
    assert lp2.totalSoldTokens == expectedWeight1;
    assert lp2.investments[alice_intent_acc] == InvestmentAmount(amount, weight2, 0);
    assert lp2.participantsCount == 1;

    // Applying discount2
    assert discount2.IsActive(timeInDiscount2);
    var bob := "bob.testnet";
    var bob_intent_acc := IntentAccount(bob);
    var amount_2 := 60000;
    var expectedWeight2 := discount2.CalculateWeightedAmount(amount_2);
    assert 66000 == expectedWeight2 == lp2.config.CalculateWeightedAmountSpec(amount_2, timeInDiscount2);
    var (lp3, amount3, weight3, refund3) := lp2.DepositSpec(bob, amount_2, bob_intent_acc, timeInDiscount2);
    assert amount3 == amount_2;
    assert weight3 == expectedWeight2;
    assert refund3 == 0;
    assert lp3.totalDeposited == amount + amount_2;
    assert lp3.totalSoldTokens == expectedWeight1 + expectedWeight2;
    assert lp3.investments[alice_intent_acc] == InvestmentAmount(amount, expectedWeight1, 0);
    assert lp3.investments[bob_intent_acc] == InvestmentAmount(amount_2, expectedWeight2, 0);
    assert lp3.participantsCount == 2;

    // Out of Discount period
    var amount_3 := 10000;
    var expectedWeight3 := amount_3;
    assert 10000 == expectedWeight3 == lp2.config.CalculateWeightedAmountSpec(amount_3, timeInDiscount2Ended);
    var (lp4, amount4, weight4, refund4) := lp3.DepositSpec(bob, amount_3, bob_intent_acc, timeInDiscount2Ended);
    ...
    assert lp4.investments[alice_intent_acc] == InvestmentAmount(amount, expectedWeight1, 0);
    assert lp4.investments[bob_intent_acc] == InvestmentAmount(amount_2 + amount_3, expectedWeight2 + expectedWeight3, 0);
    assert lp4.participantsCount == 2;
    assert lp4.IsSuccess(cfg.endDate);
